...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  const [ faster, setFaster ] = useState((localStorage.getItem('faster') === 'true')); // фикс сохранения параметров заказа
      ...
      <div className="Place__choice">
        ...
        <div className="Place__choice-item">
          ...
          <input
            ...
            onChange={event => {
              ...
              localStorage.setItem('time', event.target.value.toString());
              localStorage.setItem('faster', 'false');
            }}
            onBlur={() => {
              if (time) {
        ...
        <div className="Place__choice-item">
          ...
          <Checkbox checked={selfService} onToggle={() => {
            ...
            localStorage.setItem('selfService', (!selfService).toString());
          }} />
        </div>
        <div className="Place__choice-item">
          <h3>На месте</h3>
          <Checkbox checked={!selfService} onToggle={() => {
            ...
            localStorage.setItem('selfService', (!setSelfService).toString());
          }} />
        </div>
      </div>
